...
class Preprocessor():
    ...
    def TrainData(dataDir, batchSize, trainPercent, shuffle = True):
        '''
        ...
        for filename in os.listdir(dataDir):
            ...
            if Cfg.Normalize:
                ...
                if not os.path.exists(".//Datasets//DMax") or not os.path.exists(".//Datasets//DMin"):
                    os.mkdir(".//Datasets//DMax")
                    os.mkdir(".//Datasets//DMin")
                norm = pd.DataFrame(max)
                norm.to_csv(".//Datasets//DMax" + f"//{filename}", index = None, header = None)
                norm = pd.DataFrame(min)
                norm.to_csv(".//Datasets//DMin" + f"//{filename}", index = None, header = None)
            ...
            for i in range(0, raw.shape[0]):
                ...
                if (raw.shape[0] - i >= Cfg.Days + 1):
                    # Get the raw data.
                    rawData = raw[i:(i + Cfg.Days), Cfg.labels:,]
...
if __name__ == "__main__":
    trainData, devData = Preprocessor.TrainData(Cfg.dataDir, Cfg.batchSize, 0.9)
